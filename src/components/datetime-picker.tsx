...
export function DateTimePicker({
  ...
  // Create a default date set to 9:00 AM
  const getDefaultDate = useCallback(() => {
    const date = new Date();
    date.setHours(16, 30, 0, 0); // Set to 9:00:00.000 AM
    return date;
  }, []);

  const initDate = useMemo(() => new TZDate(value || getDefaultDate(), timezone), [value, timezone, getDefaultDate]);
  ...
  const dislayFormat = useMemo(() => {
    if (!displayValue) return 'Pick a Date & Time';
    return format(
      displayValue,
    ...
    <Popover open={open} onOpenChange={setOpen} modal={modal}>
      <PopoverTrigger asChild>
        {renderTrigger ? (
          ...
          <div
            className={cn(
              'flex w-full cursor-pointer items-center h-9 ps-3 pe-1 font-normal rounded-md text-sm shadow-sm bg-[#0f172a] border border-gray-700',
...
function TimePicker({
  ...
  // Set default hour to 9 and minute to 0 when no time is specified
  const initialHour = useMemo(() => {
    if (!value) {
      return use12HourFormat ? 9 : 9; // 9 AM in both formats
    }
    return use12HourFormat ? +format(value, 'hh') : value.getHours();
  }, [value, use12HourFormat]);

  const initialMinute = useMemo(() => {
    if (!value) return 0; // Default to 0 minutes
    return Math.round(value.getMinutes() / 5) * 5;
  }, [value]);

  const initialAmPm = useMemo(() => {
    if (!value) return AM_VALUE; // Default to AM
    return format(value, 'a') === 'AM' ? AM_VALUE : PM_VALUE;
  }, [value]);
  ...
  useEffect(() => {
    if (isUserChange.current) {
      const newDate = buildTime({ use12HourFormat, value, formatStr, hour, minute, ampm });
      onChange(newDate);
  ...
  const onHourChange = useCallback(
    (v: TimeOption) => {
      ...
      if (min || max) {
        let newTime = buildTime({ use12HourFormat, value, formatStr, hour: v.value, minute, ampm });

        if (min && newTime < min) {
          setMinute(Math.ceil(min.getMinutes() / 5) * 5);
        }

        if (max && newTime > max) {
          setMinute(Math.floor(max.getMinutes() / 5) * 5);
        }
      }
    },
  ...
  const onAmpmChange = useCallback(
    (v: TimeOption) => {
      ...
      if (min || max) {
        let newTime = buildTime({ use12HourFormat, value, formatStr, hour, minute, ampm: v.value });
  ...
  const display = useMemo(() => {
    let arr = [];
    for (const element of ['hour', 'minute']) {
...
function buildTime(options: BuildTimeOptions) {
  ...
  if (use12HourFormat) {
    ...
    date = setHours(setMinutes(setSeconds(value, 0), minute), hour);
  }
  return date;
}